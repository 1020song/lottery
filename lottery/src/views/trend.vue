			...
			<table>
				<tbody>
					<tr v-for="(i,$index) in arrList[0]" :key="$index" :style="{background:$index%2!=1?'#08533c':'#0b5f45','margin-top':$index==0?'1rem':'0'}">
						<td style="word-break:break-all">{{i}}期</td>
						<td>{{arrList[4][$index].num}}</td>
						...
						<td class="w_6" v-for="(j,index) in 6" :key="index">
							<span :class="{active:j==parseInt(arrList[1][$index][0]) || j==parseInt(arrList[1][$index][1]) || j==parseInt(arrList[1][$index][2])}">
								{{j}}
								<!-- 蓝色小圆圈 -->
								<div class="blue_two" v-if="(arrList[4][$index].blues!='' && j==arrList[1][$index][1])">{{arrList[4][$index].blues}}</div>
							</span>
						</td>
					</tr>
				</tbody>
	...
	created() {
		this.$axios.get('/indexs/index').then(res=>{
			...
			res.data.forEach((element,index) => {
				...
				if(blues[0]==blues[2] && blues[0] == blues[1]) blues1 = '3';
				else if(blues[0] == blues[1] || blues[1] == blues[2]) blues1 = '2'
				san.push({
					num:element.prizeNum.split(',')[0]+element.prizeNum.split(',')[1]+element.prizeNum.split(',')[2],
					blues:blues1
				})
			});
